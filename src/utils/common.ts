...
export const ratingValues = Array.from({ length: STARS_NUMBER }, (_, i) => i + 1).sort(
  (a, b) => b - a,
);
...
export const citiesList: Cities[] = Object.entries(Cities).map(([, value]) => value);

export const sortTypesList: SortTypes[] = Object.entries(SortTypes).map(([, value]) => value);
